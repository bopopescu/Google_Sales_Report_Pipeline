...
while start_date <= end_date:
    ...
    if len(rs) > 1 and start_date.strftime('%Y-%m') != end_date.strftime('%Y-%m'):
        ...
        call(["sudo", "gsutil", "cp", "gs://pubsite_prod_rev_02524245599547527969/sales/salesreport_%s.zip" % start_date.strftime("%Y%m") , "google_sales_data_%s.zip" % start_date.strftime("%Y%m")])
